...
/*
  ...
  Version: 1.0
  Author: Arun
  Author URI: https://www.github.com/arunshenoy99/
*/
...
class PetAdoptionTablePlugin {
  function __construct() {
    ...
    // This runs only once when the plugin is activated and has the syntax activate_plugin_dir_name/main_plugin_php_file
    // We use this to create the table only when we activate the plugin
    ...
    // Calls createpet only when a user has logged in
    add_action('admin_post_createpet', array($this, 'createPet'));
    // Calls createpet when there is no user logged in, in both cases we just wanna call createpet
  ...
  function deletePet() {
    if (current_user_can('administrator')) {
      $id = sanitize_text_field($_POST['id-to-delete']);
      global $wpdb;
  ...
  function createPet() {
    if (current_user_can('administrator')) {
      $pet = generatePet();
      $pet['petname'] = sanitize_text_field($_POST['incoming-pet-name']);
      global $wpdb;
