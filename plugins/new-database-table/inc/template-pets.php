        ...
        <?php if (current_user_can('administrator')); { ?>
          <td style="text-align: center;">
            <form action="<?php echo esc_url(admin_url('admin-post.php')); ?>" method="POST">
              ...
              <input type="hidden" name="id-to-delete" value="<?php echo $pet->id; ?>">
  ...
  <?php
    if (current_user_can('administrator')) { ?>
      <form action="<?php echo esc_url(admin_url('admin-post.php')); ?>" class="create-pet-form" method="POST">
        <p>Enter the name for a new pet.</p>
        <!-- This input field makes admin-post.php give us an action hook by the name admin_post_createpet to which we can hook on to for form handling -->
        <input type="hidden" name="action" value="createpet">
        <input type="text" name="incoming-pet-name" placeholder="Name...">
