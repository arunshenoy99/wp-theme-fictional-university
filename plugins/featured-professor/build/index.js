...
function EditComponent(props) {
  ...
  }, [props.attributes.profId]); // This useEffect runs when we delete a featured professor block
  ...
  function updateTheMeta() {
    const profsForMeta = wp.data.select('core/block-editor').getBlocks().filter(x => x.name == 'ourplugin/featured-professor').map(x => x.attributes.profId).filter((x, index, arr) => arr.indexOf(x) == index);
